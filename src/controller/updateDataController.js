...
export async function editBySuper(req, res) {
  ...
  databaseconnect()
    .then(async () => {
      await dbSuper
        .collection(`${position}`)
        .updateOne({ code: code }, { $set: {phone : data2.phone , email:data2.email , name:data2.name , code: data2.code} });
      if(position === "manager"){
        await dbSuper
        .collection("employee")
        .updateOne({ manCode: code }, { $set: { manCode: data2.code } });
        await dbSuper
        .collection("manager")
        .updateOne({ manCode: code }, { $set: { manCode: data2.code } });
        await dbUser
        .collection("data")
        .updateMany({ manCode: code } , {$set: { manCode: data2.code , manPhone:data2.phone , manEmail:data2.email , manName:data2.name } });
        await dbEmp
        .collection("empTSreq")
        .updateMany({ manCode: code } , {$set: { manCode: data2.code } });
      }
  ...
  databaseconnect()
    .then(async () => {
      ...
      response.submittedDate = date;
        ...
        let arr = [];
        response0.data.map((item) => {
          arr.push(item && item.projectCode);
        });
        const set = new Set(arr);
        const UniqueProject = set.size;
